 ...
 <!-- Profile Section: Jobseeker -->
      <?php if (isset($_SESSION['role_id']) && $_SESSION['role_id'] == 1): ?>
        <?php
          ...
          if ($id_user) {
            // Single LEFT JOIN query to get all needed info
            $sql = "
              SELECT u.*, s.name AS state_name, c.name AS city_name, e.name AS education_name
              FROM users u
      ...
      <?php if (isset($_SESSION['role_id']) && $_SESSION['role_id'] == 2): ?>
        <?php
          ...
          if ($id_company) {
            $sql = "
              SELECT co.*, s.name AS state_name, c.name AS city_name, i.name AS industry_name
              FROM company co
              LEFT JOIN states s ON co.state_id = s.id
              LEFT JOIN districts_or_cities c ON co.city_id = c.id
              LEFT JOIN industry i ON co.industry_id = i.id
              WHERE co.id_company = '$id_company'
            ";
            $query = $conn->query($sql);
            $row = $query->fetch_assoc();
        ?>
